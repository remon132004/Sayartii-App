...
public abstract class BluetoothConnection
{
    ...
    public void connect(String address, UUID uuid) throws IOException {
        ...
        // Cancel discovery BEFORE creating the socket (critical for stability)
        bluetoothAdapter.cancelDiscovery();

        BluetoothSocket socket = null;
        IOException lastErr = null;

        // Try 1: Standard secure
        try {
            socket = device.createRfcommSocketToServiceRecord(uuid);
            if (socket != null) {
                socket.connect();
            } else {
                throw new IOException("Standard socket creation returned null");
            }
        } catch (IOException e1) {
            lastErr = e1;
            try { if (socket != null) socket.close(); } catch (Exception ignored) {}

            // Try 2: Standard insecure
            try {
                socket = device.createInsecureRfcommSocketToServiceRecord(uuid);
                if (socket != null) {
                    socket.connect();
                } else {
                    throw new IOException("Insecure socket creation returned null");
                }
            } catch (IOException e2) {
                lastErr = e2;
                try { if (socket != null) socket.close(); } catch (Exception ignored) {}

                // Try 3: Reflection secure
                try {
                    ...
                    if (socket != null) {
                        ...
                        throw new IOException("Reflection secure socket creation returned null");
                    }
                } catch (Exception e3) {
                    try { if (socket != null) socket.close(); } catch (Exception ignored) {}

                    // Try 4: Reflection insecure
                    try {
                        socket = (BluetoothSocket) device.getClass()
                        ...
                        if (socket != null) {
                            ...
                            throw new IOException("Reflection insecure socket creation returned null");
                        }
                    } catch (Exception e4) {
                        try { if (socket != null) socket.close(); } catch (Exception ignored) {}
                        throw new IOException("All connection methods failed. Last error: " + lastErr.getMessage());
                    }
                }
            }
        }
